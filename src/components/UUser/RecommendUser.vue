<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>推荐用户</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图 -->
    <el-card>
      <!-- 搜索 添加 -->
      ...
      <el-table :data="userlist" border>
        ...
        <el-table-column align="center" label="安装易零通">
          <template slot-scope="scope">
            <el-button type="success" @click="setUserSetUp(scope.row.id)">安装</el-button>
          </template>
        </el-table-column>
      ...
      <el-dialog title="培育得分" :visible.sync="breedCard" width="50%">
        ...
        <el-descriptions title="满分30" direction="vertical" :column="1" border>
          ...
          <template>
            <el-descriptions-item v-if="UserDetailForm.isDisplay==1" label="具有新品陈列/重点品牌陈列/主题陈列,陈列品规数量不少于10个;摆放整齐有序(5分)">
              5 分
            </el-descriptions-item>
    ...
    methods: {
      ...
      async getUserByScore() {
        ...
        if (this.queryInfo.lowScore != 0) {
          const {
            data: res
          } = await this.$http.get('getHighUserByScore', {
